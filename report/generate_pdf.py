...
class FormalAcademicPDF(FPDF):
    def header(self):
        if self.page_no() > 1:
            ...
            self.cell(120, 5, "Laporan Remidi Cloud Computing - Mini Cloud Image Studio", border=0, align="L")
            self.cell(50, 5, f"Halaman {self.page_no()}", border=0, align="R", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            self.set_draw_color(150, 150, 150)
            self.set_line_width(0.3)
            # Horizontal rule BELOW text at Y=17mm
...
def sanitize(text: str) -> str:
    """Sanitize string for standard FPDF Times/Courier PDF encoding."""
    if not text:
        return ""
    replacements = {
        "—": " - ",
...
def build_pdf():
    ...
    pdf.set_margins(20, 22, 20)  # Top margin = 22mm to start below header line at 17mm
    ...
    # =========================================================================
    # COVER PAGE (Formal Academic Style - Clean Black Text)
    # =========================================================================
    pdf.ln(8)
    ...
    # =========================================================================
    # MARKDOWN PARSER (Wrapped Tables, Wrapped Code Blocks, Clean Headers)
    # =========================================================================
    in_code_block = False
    in_table = False
    table_rows = []
    ...
    for line in lines:
        ...
        if in_code_block:
            ...
            # Wrap long code lines to max 70 chars to avoid right-margin overflow
            wrapped_sublines = textwrap.wrap(raw_line, width=70, replace_whitespace=False, drop_whitespace=False)
            if not wrapped_sublines:
                wrapped_sublines = [""]
            
            for sub in wrapped_sublines:
                pdf.multi_cell(epw, 4.5, sanitize(f"  {sub}"), fill=True)
            continue
        ...
        if raw_line:
            ...
            pdf.multi_cell(epw, 5.5, sanitize(clean_text), align="J")
            pdf.ln(2)
    ...
    print(f"Generated formal academic PDF report: {output_pdf_path}")
